...
## >> ATM or ATP or FTP?
1. Ketika pertama kali membuka pcap, terlihat terjadi bruteforce percobaan/request login yang berasal dari IP 10.30.3.4 kepada IP 10.15.40.20 (IP korban), maka dapat dilakukan pengecekan packet dengan protocol FTP pada `ftp.pcap` menggunakan filter `ftp && ip.src == 10.15.40.20` untuk melihat response dari korban yang menandakan login attacker berhasil
...
2. Filter dengan `ip.src eq 10.30.3.1` untuk menemukan input login pembobol
...
## >> Malwleowleo
1. Untuk mengetahui malware yang dikirim bisa menggunaakan filter `tcp` lalu follow TCP stream pada username attacker
...
2. Disitu pada kode C ditemukan komen dengan bahasa `Base64`
...
## >> Secret
1. Dikarenakan pada soal ditanyakan mengenai file selain malware, kita bisa mengecek packet dengan filter `ftp` pada file `evidence.pcap` untuk melihat komunikasi yang terjadi dengan protocol ftp
2. Terlihat bahwa pada filter sebelumnya (ftp) terjadi pengiriman data, menggunakan filter `ftp-data` untuk melihat data apa saja yang ada
...
5. Dari kedua file terlihat terdapat random text dan foto berisi pesan `(mirza.jpg)`, lalu ketika dicoba submit pada netcat, didapatkan pesan rahasia attacker adalah `MIO MIRZA`
...
Pada nomor sebelumnya, terlihat juga pada `conversations`, port dari attacker adalah `80`
##### C. Endpoint yang digunakan untuk login
Endpoint dapat dilihat dari method GET atau POST yang mana menggunakan protocol http. Ketika pertama membuka packet, terlihat endpoint yang digunakan di sebelah kanan POST pada kolom info, atau bisa menggunakan filter `http.request.method == POST`, didapatkan endpoint yang digunakan adalah `/`
...
Untuk melihat tools yang digunakan, kita bisa membuka packet dengan protocol `http`. Ketika follow HTTP stream dari salah satu packet, langsung terlihat tools yang digunakan adalah `Fuzz Faster U Fool v2.0.0-dev` pada bagian User-Agent
...
1. Berdasarkan HTTP stream, kita bisa melihat bahwa terdapat informasi password is incorrect pada banyak percobaan login, respon tersebut dikirim dari IP 172.20.0.2 dan dikirimkan ke attacker, maka bisa menggunakan filter `http && ip.src == 172.20.0.2 && ip.dst == 10.33.1.154 && http contains "Found"` untuk melihat username dan password mana yang setelah dicoba oleh attacker, mendapatkan response berhasil/Found dari korban
...
## >> Malwaew
1. Ketika pertama kali pcap dibuka, terlihat pada protocol TLS, terjadi komunikasi antara web server dan client yang terenkripsi TLS, sehingga ketika digunakan filter http, tidak ada packet yang muncul, maka perlu dilakukan decrypt TLS untuk melihat seluruh komunikasi data yang ada.
